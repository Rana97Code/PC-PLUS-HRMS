...
Local_purchase_router = APIRouter()



#foreing purchase ITem Search function
async def local_suggestitm(year: int, db:Session=Depends(get_db)):
    result = db.query(Item,Hscode).join(Hscode, Item.hs_code_id==Hscode.id)\
            .filter(Hscode.schedule == "cust_tarrif", Hscode.calculate_year == year)\
...
@Local_purchase_router.post("/pcplus/api/local_purchase/get_local_item_suggestions", response_model=List[ItemSuggest])
async def suggest_items(request: Request,db:Session=Depends(get_db)):
    ...
    if len(parts) > 1:
        year = parts[0]
        part2 = parts[1]
        items = await local_suggestitm(year = year, db = db)
...
@Local_purchase_router.get("/pcplus/api/local_purchase/get_local_item_details/{item_id}", response_model=ItemDetailsModel, dependencies=[Depends(get_current_active_user)])
...
@Local_purchase_router.post("/pcplus/api/local_purchase/add-local-purchase", dependencies=[Depends(get_current_active_user)])
...
@Local_purchase_router.get("/pcplus/api/local_purchase/all_local_purchase",response_model=List[ServicePurchaseFetch], dependencies=[Depends(get_current_active_user)])
async def index(db:Session=Depends(get_db)):

    x=db.query(Purchase, Supplier).join(Supplier, Purchase.supplier_id==Supplier.id).filter(Purchase.purchase_type==1)\
