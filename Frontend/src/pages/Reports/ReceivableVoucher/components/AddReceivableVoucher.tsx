...
const AddReceivableVoucher = () => {
    ...
    const [receivable_des, setReceivableDes] = useState("");
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        const receivableVoucher = {
            challan_no: challan_no,
            receivable_des: receivable_des,
        ...
        console.log(receivableVoucher);

        if (user) {

            try {
                await axios.post(`${user.base_url}/receivableVoucher/add_receivableVoucher`, receivableVoucher, { headers })
                    .then(function (response) {
                        if (response) {
                            navigate("/pages/reports/receivableVoucher");
                        } else {
                            navigate("/pages/reports/receivableVoucher/add");
                        }
                    })
    ...
    return (
        <div>
            <div className="panel flex items-center justify-between flex-wrap gap-4">
                <h2 className="text-xl font-bold">Add Receivable Voucher</h2>
            </div>
            <div className="panel mt-6">
                <div id="forms_grid">
                    <div className="flex items-center justify-between mb-5">
                        <h3 className="font-semibold text-lg dark:text-white-light">Add New Receivable Voucher</h3>
                    </div>
                    <div className="mb-5">
                        <form className="space-y-5" onSubmit={handleSubmit} >
                            <div>
                                <div className="grid grid-cols-5 gap--x-2 gap-y-3" >
                                    <label htmlFor="receivableDescription" className='col-span-1 text-base'>Receivable Description</label>
                                    <input id="receivableDescription" type="text" placeholder="Enter Receivable Description" className="form-input py-2.5 text-base col-span-4"
                                        name="receivableDescription" onChange={(e) => setReceivableDes(e.target.value)} required />
                                </div>
                            </div>
                            ...
                            <div className="flex items-center  justify-center gap-6 pt-8">
                                ...
                                <Link to="/pages/reports/receivable_voucher/add">
...
export default AddReceivableVoucher;