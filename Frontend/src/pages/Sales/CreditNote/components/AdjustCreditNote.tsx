...
const AdjustCreditNote: React.FC = () => {
        ...
        if (dataTable) {
            dataTable.querySelectorAll('tr').forEach((row) => {
                ...
                row.querySelectorAll('td input, td select').forEach((input) => {
                    ...
                    rowData[inputElement.name || 'value'] = inputElement.value;
                    rowData[inputElement.name || 'discountRate'] = inputElement.value;
                    rowData[inputElement.name || 'discountAmount'] = inputElement.value;
                    rowData[inputElement.name || 'afterDiscount'] = inputElement.value;
                    rowData[inputElement.name || 'sd'] = inputElement.value;
                    rowData[inputElement.name || 'sdBdt'] = inputElement.value;
                    rowData[inputElement.name || 'vatableValue'] = inputElement.value;
                    rowData[inputElement.name || 'vatType'] = selectValue;
                    rowData[inputElement.name || 'vat'] = selectValue;
                    rowData[inputElement.name || 'vatBdt'] = selectValue;
                ...
                <div className="mb-5">
                    <div className="panel" id="browser_default">
                        <div className="flex items-center justify-between mb-7">
                            <h5 className="font-semibold text-lg dark:text-white-light">Adjust Credit Note</h5>
                        </div>
                        <div className="mb-5">
                            <form className="space-y-5" onSubmit={handleSubmit}>
                                <div className="grid grid-cols-1 md:grid-cols-6 gap-5">
                                    ...
                                    <div>
                                        ...
                                        <input id="customer_address" type="text" value={customerAddress} className="form-input" required disabled />
                                    </div>
                                    <div>
                                        <label htmlFor="challan_no">Challan No</label>
                                        <select id="challan_no" className="form-select text-dark col-span-4 text-sm" onChange={(e) => setchallanNo(e.target.value)} required>
                                            <option >Select Challan No</option>
                                        </select>
                                    </div>
                                    <div>
                                        <label htmlFor="challanDate">Challan Date</label>
                                        <input id="challanDate" type="date" className="form-input" value={challanDate} onChange={(e) => setChallanDate(e.target.value)} />
                                    </div>
                                    <div>
                                ...
                                <div className="grid grid-cols-1 gap-x-2 gap-y-3">
                                    ...
                                    <input id="searchInput"
                                        type="text"
                                        className="h-12 border border-slate-300 rounded z-0 focus:shadow focus:outline-none form-input text-sm col-span-2"
                                ...
                                <div className="">
                                    {filteredData.length > 0 ? (
                                        <ul id="suggestionsList" className="bg-white rounded-lg shadow-lg p-3 cursor-pointer">
                                            {filteredData.map((item, index) => (
                                                <li key={index} className="border-gray-200 py-2">
                                                    {item}
                                                </li>
                                            ))}
                                        </ul>
                                    ) : (
                                        searchTerm && <div className="bg-white rounded-lg shadow-lg p-3">No results found</div>
                                    )}
                                    ...
                                    <ul style={{ cursor: 'pointer' }} className="mt-4 ml-20 w-1/2 absolute bg-slate-300" id="suggestionsList"></ul> */}
                                    ...
                                    <table id="dataTable" className="whitespace-nowrap table-hover border dataTable">
                                        <thead>
                                            <tr className="whitespace-nowrap border overflow-x-auto">
                                                ...
                                                <th className="w-10 border-x-1 border-black" >Amount</th>
                                                <th className="w-6" >VAT Rate</th>
                                                <th className="w-10 border-x-1 border-black" >VAT</th>
                                                <th className="w-6" >SD</th>
                                                <th className="w-9 border-x-1 border-black" >Return Qty</th>
                                                <th className="w-6" >Return Amount</th>
                                                <th className="w-9 border-x-1 border-black" >Return VAT</th>
                                                <th className="w-6" >Return SD</th>
                                ...
                                <div className="grid grid-cols-5 gap--x-2 gap-y-3">
                                    <label htmlFor="reason_issue" className='col-span-1 text-sm'>Reason of Issue</label>
                                    <textarea id="reason_issue" placeholder="Reason of Issue" className="form-input py-2.5 text-sm col-span-4" onChange={(e) => setNote(e.target.value)} />
                                </div>
                                <div className="grid grid-cols-5 gap--x-2 gap-y-3">
...
export default AdjustCreditNote;
